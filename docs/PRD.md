...
**FR-0.3 Packaging.** `uv` workspace stays; `uv sync` alone must produce a working dev environment; `uv.lock` committed; `tinyic` console entry point via `[project.scripts]`; `uvx tinyic` works from a clean machine.
