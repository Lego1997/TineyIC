...
**The v2 revamp and v2.1 provider matrix have landed**, as has the **v2.2 amendment** (PRD §16): the Textual debate face is replaced by the secured local Web Town Hall, replay uses that same read-only page, and the cited Persona Factory adds layered user personas. Providers remain `openai · anthropic · grok · google · kimi · ollama`; the Textual onboarding wizard remains. The source of truth for behavior is `docs/PRD.md` (requirements/amendments), `docs/event-schema.md` (the engine↔renderer contract), and `AGENTS.md` (the headless agent contract). Historical review/kickoff records stay verbatim.
