...
- **The Grok lane never writes `~/.grok/auth.json`.** The read-through reuses the official grok CLI's sign-in strictly read-only; stale tokens refresh in memory per session (taking ownership of a copied refresh token can log the user's own CLI out — same hazard as the Codex lane). xAI enforces entitlement server-side: the 403 is the stable reason code `subscription_inactive`, which advances auth rotation.
